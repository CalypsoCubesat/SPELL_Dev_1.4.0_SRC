		...
		if (repository.getProperty(ITasksCoreConstants.PROPERTY_USE_SECURE_STORAGE) == null
			...
			try {
				AuthenticationCredentials creds = repository.getCredentials(AuthenticationType.REPOSITORY),
						httpCreds = repository.getCredentials(AuthenticationType.HTTP),
						proxyCreds = repository.getCredentials(AuthenticationType.PROXY);
				...
				try {
					...
					//SecurePreferencesFactory.open(new URL(repository.getUrl()), null).flush();
				...
				StatusHandler.log(new Status(IStatus.ERROR, ITasksCoreConstants.ID_PLUGIN,
						NLS.bind("Could not migrate credentials to secure storage for {0}", repository.getUrl()), e));
			}
		}
		return false;
	}
