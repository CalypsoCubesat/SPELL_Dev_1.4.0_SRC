...
then
    ...
    export SPELL_DATA=$SPELL_DEV_HOME/data
fi
[[ ! -d $SPELL_DATA ]] && echo "ERROR: cannot find SPELL data directory: $SPELL_DATA" && exit 1
echo "SPELL data: $SPELL_DATA"

# SPELL config
if [[ -z "$SPELL_DEV_CONFIG" ]]
then
    echo "[!] WARNING: SPELL_DEV_CONFIG variable not defined, using default"
    export SPELL_DEV_CONFIG=$SPELL_DEV_HOME/config
fi
[[ ! -d $SPELL_DEV_CONFIG ]] && echo "ERROR: cannot find SPELL config directory: $SPELL_DEV_CONFIG" && exit 1
echo "SPELL config: $SPELL_DEV_CONFIG"

# SPELL runtime data
if [[ -z "$SPELL_SYS_DATA" ]]
then
    echo "[!] WARNING: SPELL_SYS_DATA variable not defined, using default"
    export SPELL_SYS_DATA=$SPELL_DEV_HOME/data
fi
...
then
    ...
    export SPELL_LOG=$SPELL_DEV_HOME/log
fi
...
# Update python path
export PYTHONPATH=${PYTHONPATH}:${SPELL_DEV_HOME}/spell:${SPELL_DEV_HOME}/server:${SPELL_DEV_HOME}
export TK_LIBRARY=""
export TCL_LIBRARY=""
export PYTHONCASEOK=""
