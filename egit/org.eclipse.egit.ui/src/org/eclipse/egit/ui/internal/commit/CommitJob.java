...
public class CommitJob extends Job {
	 ...
	 * @param b
	 */
	public void setDoPullFirst(boolean b){
		this.doPullFirst = b;
	}
	...
	private void pushUpstream(final RevCommit commit, final PushMode pushTo) {
		...
		if (pushTo == PushMode.GERRIT || config == null) {
			...
			final PullOperation pullOperation = new PullOperation(repositories,
					timeout);

			Map<Repository, Object> results = pullOperation.getResults();
			pullOperation.setCredentialsProvider(new EGitCredentialsProvider());
			results.clear();
			Job pullJob = new Job(
					"Pulling " + repository.getDirectory().getName()) {
