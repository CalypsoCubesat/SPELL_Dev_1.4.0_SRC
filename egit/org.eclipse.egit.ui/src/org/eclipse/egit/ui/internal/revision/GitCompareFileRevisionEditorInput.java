...
public class GitCompareFileRevisionEditorInput extends SaveableCompareEditorInput {
	...
	private DiffNode compare(ITypedElement actLeft, ITypedElement actRight, ITypedElement actAncestor) {
		if (actLeft.getType().equals(ITypedElement.FOLDER_TYPE)) {
			...
			DiffNode diffNode = new NotifiableDiffNode(null, Differencer.CHANGE,
			...
			while (li<lc.length && ri<rc.length) {
				...
				if (compareTo == 0) {
					...
					DiffNode childDiffNode = new NotifiableDiffNode(null,
							Differencer.ADDITION, an, ln, null);
					...
					DiffNode childDiffNode = new NotifiableDiffNode(null,
							Differencer.DELETION, an, null, rn);
					diffNode.add(childDiffNode);
			...
			if (actAncestor != null)
				return new NotifiableDiffNode(null, Differencer.CONFLICTING,
						actAncestor, actLeft, actRight);
			else
				return new NotifiableDiffNode(null, Differencer.CHANGE, null,
						actLeft, actRight);
		}
	}
