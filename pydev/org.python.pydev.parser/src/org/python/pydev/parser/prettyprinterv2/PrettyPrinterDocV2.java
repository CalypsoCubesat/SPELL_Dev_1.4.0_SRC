            ...
            if (linePart instanceof ILinePart2) {
                ...
                for (int i = 0; i < replacements.length; i += 2) {
                    ...
                    if (iLinePart2.getString().equals(toReplace)) {
                        ...
                        replaced += 1;
                        if (newToken.isEmpty()) {
                            PrettyPrinterDocLineEntry line = getLine(iLinePart2.getLine());
                            line.removeLine(iLinePart2);
                            if (line.isEmpty()) {
                                linesToColAndContents.remove(iLinePart2.getLine());
                            }
                            break;
                        }
                    }
                }
            } else if (linePart instanceof ILinePart) {
                for (int i = 0; i < replacements.length; i += 2) {
                    String toReplace = replacements[i];
                    String newToken = replacements[i + 1];
                    if (linePart.toString().equals(toReplace)) {
                        if (linePart instanceof LinePartRequireMark) {
                            PrettyPrinterDocLineEntry line = getLine(linePart.getLine());
                            line.removeLine(linePart);
                            if (line.isEmpty()) {
                                linesToColAndContents.remove(linePart.getLine());
                            }
                            replaced += 1;
                            break;
                        } else if (linePart instanceof LinePartIndentMark) {
                            PrettyPrinterDocLineEntry line = getLine(linePart.getLine());
                            line.removeLine(linePart);
                            if (line.isEmpty()) {
                                linesToColAndContents.remove(linePart.getLine());
                            ...
                            replaced += 1;
                            break;
                        } else if (linePart instanceof LinePartRequireIndentMark) {
                            PrettyPrinterDocLineEntry line = getLine(linePart.getLine());
                            line.removeLine(linePart);
                            if (line.isEmpty()) {
                                linesToColAndContents.remove(linePart.getLine());
