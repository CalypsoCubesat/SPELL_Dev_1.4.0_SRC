        ...
        try {
            ...
            try {
                if (Display.getCurrent() != null) {
                    final SimpleNode[] result = { null };
                    final Exception[] ex = { null };

                    final IGrammar g = grammar;
                    Thread parserThread = new Thread("Python Parser Thread") {
                        @Override
                        public void run() {
                            try {
                                long start = System.currentTimeMillis();
                                result[0] = g.file_input();
                                System.out.println("Parsing of " + info.moduleName + " took "
