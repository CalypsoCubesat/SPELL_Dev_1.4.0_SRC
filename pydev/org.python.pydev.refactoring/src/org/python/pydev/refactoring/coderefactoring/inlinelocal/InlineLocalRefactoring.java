    ...
    private boolean isValid(List<Name> variables) {
        ...
        for (Name variable : variables) {
            ...
            if (variable.ctx == expr_contextType.Param || variable.ctx == expr_contextType.KwOnlyParam) {
                status.addFatalError(Messages.format(Messages.inlineLocalParameter, variable.id));
                return false;
            }
        }

        if (assignCounter > 1) {
            status.addFatalError(Messages.format(Messages.inlineLocalMultipleAssignments, variables.get(0).id));
            return false;
        }
