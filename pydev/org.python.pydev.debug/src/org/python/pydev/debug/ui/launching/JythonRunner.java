...
import org.python.pydev.debug.ui.JythonUIBridge;
...
public class JythonRunner {
    ...
    public static void run(final PythonRunnerConfig config, ILaunch launch, IProgressMonitor monitor)
        ...
        try {
            ...
            if (config.isDebug) {
                ...
                doIt(config, monitor, config.envp, config.getCommandLine(true, launch.getLaunchConfiguration()),
    ...
    private static void runDebug(final PythonRunnerConfig config, final ILaunch launch, IProgressMonitor monitor)
        ...
        String[] cmdLine = config.getCommandLine(true, launch.getLaunchConfiguration());
        int localPort = config.getDebuggerListenConnector().getLocalPort();
        Registry registry = LocateRegistry.createRegistry(localPort + 10);
        JythonUIBridge bridge = new JythonUIBridge(registry);

        System.out.println("Created registry on port " + (localPort + 10));
        Remote stub = UnicastRemoteObject.exportObject(bridge, localPort + 10);
        bridge.setStub(stub);
        try {
            registry.bind("JythonUIBridge", stub);
