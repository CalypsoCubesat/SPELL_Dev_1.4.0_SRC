...
public final class OccurrencesVisitor extends AbstractScopeAnalyzerVisitor {
    ...
    public Object visitStr(Str node) throws Exception {
        ...
        if (node.s != null && node.s.startsWith("T ") && !"T ".equals(node.s)) {
            ISpellDatabase database = DatabaseManager.getInstance().getProjectDatabase(nature.getProject());
            if (database != null && !database.getTelemetryParameterNames().isEmpty()) {
                if (!database.isTelemetryParameter(node.s.split(" ")[1])) {
                    int startInternalStrColOffset = 1; // +1 for 'f' and +1 for the quote.
                    if (node.raw) {
                        startInternalStrColOffset += 1;
                    }
                    if (node.unicode) {
                    ...
                    IDocument doc = new Document(node.s);
                    IRegion region = document.getLineInformation(node.beginLine - 1);
                    int offset = node.beginColumn + node.s.length() + startInternalStrColOffset;

                    String character = "";
                    do {
                        try {
                            character = document.get(region.getOffset() + offset, 1);
                        } catch (Exception e) {
                            character = null;
                        }
                        ...
                        offset++;
                            ...
                            && character.equals(" ")
                            && offset < node.beginColumn + node.s.length() + 100);
                    if (character == null || !(character.equals("+") || character.equals("%"))) {
                        if (node.s.split(" ")[1].length() >= 4) {
                            if (!document.get(region.getOffset(), region.getLength()).contains("@IgnoreError")) {
                                reportMiscError(node, node.beginLine, node.beginColumn + startInternalStrColOffset,
                                        node.beginColumn + node.s.length() + startInternalStrColOffset, doc,
                                        "'" + node.s.split(" ")[1] + "' is not a valid TM!");
                            }
        ...
        if (node.s != null && node.s.startsWith("C ") && !"C ".equals(node.s)) {
            ISpellDatabase database = DatabaseManager.getInstance().getProjectDatabase(nature.getProject());
            if (database != null && !database.getTelecommandNames().isEmpty()) {
                if (!database.isTelecommand(node.s.split(" ")[1])) {
                    int startInternalStrColOffset = 1; // +1 for 'f' and +1 for the quote.
                    if (node.raw) {
                        startInternalStrColOffset += 1;
                    }
                    ...
                    do {
                        ...
                        offset++;
                            ...
                            && offset < node.beginColumn + node.s.length() + startInternalStrColOffset + 100);
                    if (character == null || !(character.equals("+") || character.equals("%"))) {
                        if (node.s.split(" ")[1].length() >= 4) {
                            if (!document.get(region.getOffset(), region.getLength()).contains("@IgnoreError")) {
                                reportMiscError(node, node.beginLine, node.beginColumn + startInternalStrColOffset,
                                        node.beginColumn + node.s.length() + startInternalStrColOffset, doc,
                                        "'" + node.s.split(" ")[1] + "' is not a valid TC!");
                            }
    ...
    private void reportMiscError(Str node, int startInternalStrLineOffset, int startInternalStrColOffset,
            int endColumn, IDocument doc, String messageStr) {
        //line

        int startLine = startInternalStrLineOffset;
        int endLine = startInternalStrLineOffset;

        //col
        int startCol = startInternalStrColOffset;
        int endCol = endColumn;

        Message message = new Message(IAnalysisPreferences.TYPE_ERROR,
                messageStr, startLine, endLine, startCol, endCol, prefs);
        messagesManager.addMessage(
                AbstractVisitor.makeToken(node, this.moduleName, nature), message);
