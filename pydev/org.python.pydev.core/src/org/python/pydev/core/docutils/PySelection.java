...
public final class PySelection extends TextSelectionUtils {
    ...
    public static ActivationTokenAndQual getActivationTokenAndQual(IDocument doc, int documentOffset,
        ...
        int foundTMTCOffset = -1;
        int currentCursor = documentOffset;
        int tmtcOffset = documentOffset - 1;
        if (tmtcOffset > 0 && tmtcOffset < doc.getLength()) {

            try {
                int count = 0;
                while (tmtcOffset > 0 && count < 100) {
                    tmtcOffset--;
                    count++;
                    try {
                        char c1 = doc.getChar(tmtcOffset);
                        char c2 = doc.getChar(tmtcOffset + 1);
                        char c3 = doc.getChar(tmtcOffset + 2);

                        if (c1 == '\'' && c2 == 'T' && c3 == ' ') {
                            foundTMTCOffset = tmtcOffset + 1;
                            documentOffset = foundTMTCOffset;
                            break;
                        } else if (c1 == '\'' && c2 == 'C' && c3 == ' ') {
                            foundTMTCOffset = tmtcOffset + 1;
                            documentOffset = foundTMTCOffset;
        ...
        if (foundTMTCOffset > 0) {
            tmtcOffset = foundTMTCOffset;
            try {
                int length = currentCursor - foundTMTCOffset;
                int fullLength = length;
                while (fullLength > 0 && fullLength < 200) {

                    char c1 = doc.getChar(tmtcOffset + fullLength);
                    if (c1 == '\'') {
                        break;
                    }
                    fullLength++;
                ...
                String token = doc.get(foundTMTCOffset, length);
                ...
                return new ActivationTokenAndQual(token, qualifier, changedForCalltip, alreadyHasParams,
                        isInMethodKeywordParam, fullLength, foundTMTCOffset);
